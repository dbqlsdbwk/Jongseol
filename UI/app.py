...
def evaluate_checklist_item(text, item):
    ...
    if len(required_hits) == 0:
        required_hit_text = "없음"
    else:
        required_hit_text = ", ".join(required_hits)

    if len(optional_hits) == 0:
        optional_hit_text = "없음"
    else:
        optional_hit_text = ", ".join(optional_hits)

    if len(evidence) == 0:
        evidence_text = "관련 문장 발견 안 됨"
    else:
    ...
    return {
        ...
        "발견된 필수키워드": required_hit_text,
        "발견된 보조키워드": optional_hit_text,
        "근거 문장": evidence_text,
        "수정권고": recommendation
    }
...
def save_excel(checklist_df, vague_df):
    ...
    output_path = os.path.join(OUTPUT_DIR, "시방서_체크리스트_검토결과_" + now + ".xlsx")
...
st.write("일반시방서 PDF를 업로드하면 체크리스트별 충족 / 미흡 / 누락 여부를 자동 판단합니다.")
...
with st.sidebar:
    ...
    st.write("**충족**")
    st.write("- 필수 키워드가 모두 확인됨")
    st.write("- 보조 키워드가 1개 이상 확인됨")

    st.write("**미흡**")
    st.write("- 필수 키워드가 일부만 확인됨")

    st.write("**누락**")
    st.write("- 필수 키워드가 거의 확인되지 않음")

    st.markdown("---")
    st.info("현재 버전은 키워드 기반 Rule-Based 판단 방식입니다.")
...
if uploaded_pdf is not None:
    ...
    col1.metric("총 검토항목", str(total_count) + "개")
    col2.metric("충족", str(satisfied_count) + "개")
    col3.metric("미흡", str(insufficient_count) + "개")
    col4.metric("누락", str(missing_count) + "개")
    col5.metric("모호표현", str(vague_count) + "개")

    st.markdown("---")

    st.markdown("## 판단 결과 차트")
    ...
    if len(problem_df) == 0:
        ...
        for idx, row in problem_df.iterrows():
            with st.expander("[" + row["판단"] + "] " + row["카테고리"] + " - " + row["검토항목"]):
    ...
    st.info("PDF 파일을 업로드하면 검토가 시작됩니다.")
